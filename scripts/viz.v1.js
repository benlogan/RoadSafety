!function(){
  ...
  viz.gg = function(){
	  ...
	  var def={
        ...
        ,startAngle:-1.5*math_pi, endAngle:0.5*math_pi
	  ...
	  function gg(_){
        ...
        _.each(function() {
            ...
            var rot=gg.scale()(gg.value())*180/math_pi+90;
	  ...
	  gg.scale = function(){ 
		//return d3.scale.linear().domain(gg.domain())
          return d3.scaleLinear().domain(gg.domain())
	  ...
	  gg.setNeedle =function(a){
		//var newAngle=gg.scale()(a)*180/π+90
          var newAngle=gg.scale()(a)*180/math_pi+90
			//,oldAngle=gg.scale()(gg.value())*180/π+90
            ,oldAngle=gg.scale()(gg.value())*180/math_pi+90
			,d3ease = gg.ease();
		...
		g.selectAll(".needleshadow").data([a])
            ...
            //.each(function(){angle=a;});
            //.each(function(){angle=a;});
            //.on("end", function(){angle=a;});
  ...
  function ch(_){
    ...
    _.each(function() {
	  ...
	  labels.filter(function(d){ return d.type=="g"})
        ...
        .style("text-anchor",function(d){var a =angle(d); return a < math_pi2 || a>math_theta-math_pi2 ? "start" : "end";})
  ...
  function isBottom(d){
    //return angle(d) < 1.5*π && angle(d) >= .5*π ? 1: 0;
      return angle(d) < 1.5*math_pi && angle(d) >= .5*math_pi ? 1: 0;
  }
  function relayout(){
	keys =[];
  ...
  function relayouts(fixedSource){
	...
	groups.forEach(function(g,i){
	  ...
	  var f = (mid(g) < sm) ;
	  //g1.startAngle-=(f ? τ:0);
        g1.startAngle-=(f ? math_theta:0);
	  //g1.endAngle -= (f ? τ:0); 
        g1.endAngle -= (f ? math_theta:0); 
	});
  ...
  function viz_reduceAngle(a){
    //while(a>τ) a-=τ;
      while(a>math_theta) a-=math_theta;
    //while(a<0) a+=τ;
      while(a<0) a+=math_theta;
    return a;
  }
  ...
  function viz_arc(x){
    ...
    //return ["M",ss,"A",x[0],x[0],"0",(x[3]-x[2] > π?1:0),"1",se, "L",ee,
      return ["M",ss,"A",x[0],x[0],"0",(x[3]-x[2] > math_pi?1:0),"1",se, "L",ee,
          //"A",x[1],x[1],"0",(x[3]-x[2] > π?1:0),"0",es, "z"].join(" ");
              "A",x[1],x[1],"0",(x[3]-x[2] > math_pi?1:0),"0",es, "z"].join(" ");
  }
  function viz_chord(rs, ss, se, re, es, ee) {
      ...
      function arc(r, p, a) {  return "A" + r + "," + r + " 0 " + +(a > math_pi) + ",1 " + p;   }
	  function p(r,a){ return [r*Math.cos(a), r*Math.sin(a)]; }
      function eq(a, b){ return a.a0 == b.a0 && a.a1 == b.a1; }
      function curve(p1,a0,a1, r) {
        //a1=a1+ (a1<a0?τ:0);
          a1=a1+ (a1<a0?math_theta:0);
	    var a=a1-a0;
	    //var t=1-(a > π ? τ-a : a)/π;
          var t=1-(a > math_pi ? math_theta-a : a)/math_pi;
	    t=Math.pow(t,5);
	    //var a2 = (a1+a0)/2 -(a1-a0>π ? π : 0);
          var a2 = (a1+a0)/2 -(a1-a0>math_pi ? math_pi : 0);
